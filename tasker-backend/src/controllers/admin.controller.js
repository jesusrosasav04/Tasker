...
const getTareaDetalle = async (req, res) => {
  ...
  try {
    const [tareas] = await db.query(
      `SELECT
        t.id, t.titulo, t.descripcion, t.presupuesto,
        t.ubicacion, t.estado, t.created_at, t.updated_at,
        c.nombre AS categoria,
        u.nombre AS cliente_nombre,
    ...
    // Postulaciones de esta tarea
    const [postulaciones] = await db.query(
      `SELECT
        p.id, p.estado, p.precio_propuesto, p.mensaje, p.created_at,
    ...
    return success(res, { ...tareas[0], postulaciones });
  } catch (err) {
    console.error(err);
    return error(res, "Error al obtener detalle de la tarea", 500);
  }
};
...
const eliminarTarea = async (req, res) => {
  ...
  try {
    ...
    if (tarea[0].estado === "en_progreso")
      return error(
        res,
        "No puedes eliminar una tarea en progreso. Cancélala primero.",
        400
      );

    // Eliminar en cascada: postulaciones, luego la tarea
    await db.query("DELETE FROM postulaciones WHERE tarea_id = ?", [id]);
    await db.query("DELETE FROM tareas WHERE id = ?", [id]);

    return success(res, null, `Tarea "${tarea[0].titulo}" eliminada correctamente`);
  } catch (err) {
    console.error(err);
    return error(res, "Error al eliminar la tarea", 500);
  }
};

// PATCH /api/admin/tareas/:id/estado — cambiar estado de una tarea
const cambiarEstadoTarea = async (req, res) => {
  ...
  try {
    const [tarea] = await db.query(
      "SELECT id, estado FROM tareas WHERE id = ?",
      [id]
    );
    ...
    await db.query("UPDATE tareas SET estado = ? WHERE id = ?", [estado, id]);

    return success(res, { estado }, "Estado actualizado correctamente");
  } catch (err) {
    console.error(err);
    return error(res, "Error al cambiar estado de la tarea", 500);
  }
};
